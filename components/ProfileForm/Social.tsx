...
const Social = () => {
    ...
    const socialData = SocialData[selectedProvider] || []
        ...
        <Card className=' w-full p-0 flex flex-col border-none gap-4'>
            ...
            <CardContent className='p-0 mt-4 grid grid-cols-2 gap-4'>
                {socialData.map((social) => (
                    <Card key={social.id} className='flex flex-col justify-center items-center p-2 h-28 cursor-pointer shadow-md'>
                        <img
                            src={selectedProvider === 'shieldIcons' ? `${social.url.split('?')[0]}?style=${selectedStyle}&${social.url.split('?')[1] || ''}` : social.url}
                            alt={social.label}
                            className='min-w-16 min-h-7 rounded-md self-start '
                        />
                        <Input
                            value={social.href(usernames[social.id] || '')}
                            onChange={(e) => handleUsernameChange(social.id, e.target.value.split('/').pop())}
